Templar.component('Codedown',{
  ...
  processMultiLineCode : function(node, syntaxHighlightRules){
    ...
    var syntaxHighlight = function(text, Rule){
        var  matches = null,
            token = null,
            output = text;
         ...
         while((matches = Rule.regex.exec(text)) != null){
            token = Rule.transform.call(this, matches, Rule);
            output = output.replace(new RegExp(token.token.replace(/[-[\]{}()*+?.,\\^$|#\s]/g, "\\$&"),'g'), 
                                Rule.template.replace('%%',token.tranformation));
    ...
    while((matches = codeRX.exec(node.innerHTML)) != null){
      ...
      for(var x = 0; x < syntaxHighlightRules.length; x++){
        highlightedText = syntaxHighlight(highlightedText, syntaxHighlightRules[x]);
      }
  ...
  onCreate : function(self){
    ...
    var processMultiLineCode = function(node, syntaxHighlightRules){
        var codeRX = /```([^`]+)+?```/gm,
            matches = null,
            output = node.innerHTML,
            highlightedText = '';
    ...
    contentNode.innerHTML = syntaxHighlight(contentNode.innerHTML, HighlightRules.singleLineCode);
  }
